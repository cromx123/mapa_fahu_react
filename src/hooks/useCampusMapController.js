  ...
  export default function useCampusMapController() {
    ...
    // --- Favoritos: helpers consistentes ---
    useEffect(() => {
      // carga inicial
      try {
    ...
    const toggleFavorito = useCallback((place) => {
      if (!place?.id) return;
      const prev = JSON.parse(localStorage.getItem("favoritos") || "[]");
      const arr = Array.isArray(prev) ? prev : [];
      const exists = arr.some((f) => f.id === place.id);
      const next = exists
        ? arr.filter((f) => f.id !== place.id)
        : [...arr.filter((f) => f.id !== place.id), {
            id: place.id,
            name: place.name,
            lat: place.lat,
